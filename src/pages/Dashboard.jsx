...
import { FiPackage, FiUser, FiShoppingBag, FiChevronRight, FiClock, FiCheckCircle, FiMapPin, FiMail, FiPhone, FiLogOut, FiSettings } from 'react-icons/fi'
import { Link, useNavigate } from 'react-router-dom'

export default function Dashboard() {
  const { orders, cartItems, cartSubtotal } = useCart()
                ...
                <div className="space-y-6">
                  ...
                  {orders.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {orders.map((order) => (
                        <div key={order.id} className="border border-neutral-100 rounded-2xl p-4 sm:p-5 hover:border-neutral-200 transition-colors">
                          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                            ...
                            <div className="text-right">
                              ...
                              <p className="font-bold text-neutral-900">€{order.total.toFixed(2).replace('.', ',')}</p>
                            </div>
                          </div>
              ...
              {activeTab === 'profile' && (
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-4 p-4 rounded-2xl bg-neutral-50 border border-neutral-100">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-bold text-neutral-900">
                              {user?.user_metadata?.first_name ? `${user.user_metadata.first_name} ${user.user_metadata.last_name || ''}` : 'N/A'}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-4 p-4 rounded-2xl bg-neutral-50 border border-neutral-100">
                        ...
                        <div className="flex items-center gap-4 p-4 rounded-2xl bg-neutral-50 border border-neutral-100">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-bold text-neutral-900">+1 234 567 890</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      <h4 className="text-xs font-bold text-neutral-400 uppercase tracking-widest">Shipping Address</h4>
                      <div className="p-5 rounded-3xl border-2 border-neutral-900 relative">
                        <div className="absolute top-4 right-4 h-5 w-5 rounded-full bg-neutral-900 flex items-center justify-center text-white">
                          <FiCheckCircle className="h-3 w-3" />
                        </div>
                        <div className="flex items-start gap-4 mb-4">
                          <FiMapPin className="h-5 w-5 text-neutral-900 mt-1" />
                          <div>
                            <p className="text-sm font-bold text-neutral-900 mb-1">Home Address</p>
                            <p className="text-xs text-neutral-500 leading-relaxed">
                              123 Ostren Street, Suite 456<br />
                              New York, NY 10001<br />
                              United States
                            </p>
                          </div>
                        </div>
                        <button className="text-xs font-bold text-neutral-900 underline underline-offset-4">
                          Edit Address
                        </button>
                      </div>
                    </div>
                      ...
                      <div className="pt-6 border-t border-neutral-100">
                        <div className="flex items-center justify-between mb-6">
                          <div>
                            ...
                            <p className="text-2xl font-black text-neutral-900">€{cartSubtotal.toFixed(2).replace('.', ',')}</p>
                          </div>
                          <Link 
                            to="/products"
                        ...
                        <button 
                          ...
                          onClick={() => {
                            // This would ideally open the cart drawer and proceed to checkout
